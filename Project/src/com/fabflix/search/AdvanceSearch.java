...
public class AdvanceSearch extends HttpServlet {
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String reqSearch = title+director+year+firstName+lastName;
		if (currentSearch == null)
			currentSearch = reqSearch;
		...
		if(!command.equals(req) || page < 1 || !currentSearch.equals(reqSearch)){
			page = 1;
			currentSearch = reqSearch;
			command = req;
		}
		...
		try {
			movies = query.advanceSearch(title, year, director, firstName, lastName, order, offset, sortSession);
		...
		session.setAttribute("currentSearch", currentSearch);
		request.getRequestDispatcher("advanceSearchShow.jsp").forward(request, response);
	}
